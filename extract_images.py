#!/usr/bin/env python3
"""Extrator de imagens de PDF com modo robusto opcional e relatório detalhado."""
...
def _extract_metadata(dictionary_bytes: bytes) -> dict[str, Any]:
    values: dict[str, Any] = {"Width": None, "Height": None, "BitsPerComponent": None, "ColorSpace": None}
...
def _choose_output(decoded: bytes, filters: list[str], meta: dict[str, Any]) -> tuple[bytes, str]:
    ...
    png = _raw_to_png(decoded, meta["Width"], meta["Height"], meta["ColorSpace"], meta["BitsPerComponent"])
    if png is not None:
        return png, "png"
    tiff = _raw_to_tiff_gray(decoded, meta["Width"], meta["Height"], meta["BitsPerComponent"])
    if tiff is not None:
        return tiff, "tiff"
    return decoded, "bin"
    ...
    for page_idx, page in enumerate(reader.pages, start=1):
        for idx, image in enumerate(page.images, start=1):
            ...
            images.append({
                ...
                "meta": {"Width": None, "Height": None, "BitsPerComponent": None, "ColorSpace": None},
                "preferred_ext": ext,
            })
    return images
    ...
    for item in images:
        try:
            ...
            if len(decoded) < min_size:
                ...
                out_file = None
                out_size = 0
            elif only_format and ext.lower() not in only_format:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--min-size", type=int, default=0, help="Tamanho mínimo em bytes para salvar saída.")
